        ...
        if self._handle is None:
            ...
            if event.type in {'MOUSEMOVE', 'LEFTMOUSE'}:
                ...
                if event.type == 'LEFTMOUSE' and end_point != start_point: # Finish updating the line and paint the vertices
                    ...
                    isolate = get_isolated_channel_ids(context.active_object.data.color_attributes.active)
...
class VERTEXCOLORMASTER_OT_RandomizeMeshIslandColorsPerChannel(bpy.types.Operator):
    ...
    def execute(self, context):
        obj = context.active_object
        mesh = obj.data
        isolate = get_isolated_channel_ids(mesh.color_attributes.active)
        if isolate is not None:
...
class VERTEXCOLORMASTER_OT_BlurChannel(bpy.types.Operator):
    ...
    def execute(self, context):
        obj = context.active_object
        mesh = obj.data
        vcol = mesh.color_attributes.active if mesh.color_attributes else mesh.color_attributes.new("Col", 'FLOAT_COLOR', 'POINT')
...
class VERTEXCOLORMASTER_OT_IsolateChannel(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        iso_vcol = mesh.color_attributes.new("Col", 'FLOAT_COLOR', 'POINT')
        ...
        mesh.color_attributes.active = iso_vcol
        brush = context.tool_settings.vertex_paint.brush
...
class VERTEXCOLORMASTER_OT_ApplyIsolatedChannel(bpy.types.Operator):
    ...
    def poll(cls, context):
        ...
        if obj is not None and obj.type == 'MESH' and obj.data.color_attributes is not None:
            vcol = obj.data.color_attributes.active
    ...
    def execute(self, context):
        ...
        iso_vcol = mesh.color_attributes.active
...
class VERTEXCOLORMASTER_OT_FlipBrushColors(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if context.object.mode == 'VERTEX_PAINT' and obj is not None and obj.type == 'MESH' \
            and get_isolated_channel_ids(context.active_object.data.color_attributes.active) is not None \
